...
class CNN_LSTM:
    ...
    def loadLSTMModel(self, modelFolder):
        ...
        print("!!!!loadLSTMMode NUM FEATURES ", self.num_features)
        checkpoint = torch.load(os.path.join(modelFolder, "lstm.pth"), map_location="cpu")
        self.lstm_model.load_state_dict(checkpoint["model"], strict=True)
        self.lstm_model.to(self.device)
        return self.lstm_model

    def predict(self, image, tool_tensor):
        self.cnn_model.eval()
        self.lstm_model.eval()
        with torch.no_grad():
            img_tensor = self.transforms(image.resize((224, 224)))
            image = torch.from_numpy(numpy.array([img_tensor])).to(self.device)
            tool_tensor = torch.tensor(tool_tensor, dtype=torch.float32).unsqueeze(0).to(self.device)
            preds = self.cnn_model((image, tool_tensor))
            ...
            taskPrediction = self.lstm_model(torch.from_numpy(expanded_sequence).float().to(self.device))
            taskPrediction = torch.softmax(taskPrediction[:, -1, :], dim=1)
            class_num = torch.argmax(taskPrediction, dim=1).cpu().numpy()[0]
    ...
    def createLSTMModel(self, num_features, num_classes):
        print("!!!! createLSTMModel num_features", num_features)
        self.lstm_model = WorkflowLSTM(num_features, num_classes)
        return self.lstm_model

class ResNet_FeatureExtractor(nn.Module):
    def __init__(self, num_output_features, num_classes, return_head=True):
        ...
        print("!! ResNet_FeatureExtractor num_output_features", num_output_features)
        self.linear1 = nn.Linear(num_features + 32, num_output_features)  # 8 tools × 4 values
        self.relu = nn.ReLU()
        self.output_head = nn.Linear(num_output_features, num_classes)
        self.return_head = return_head

    def forward(self, inputs):
        x, tool_tensor = inputs
        x = self.resnet(x)
        x = torch.cat([x, tool_tensor], dim=1)
        x = self.linear1(x)
        x = self.relu(x)
        if self.return_head:
...
class ResNet_FeatureExtractor(nn.Module):
    def __init__(self, num_output_features, num_classes, multitask=False, num_tools=10, return_head=True):
        super(ResNet_FeatureExtractor, self).__init__()
        weights = models.ResNet50_Weights.DEFAULT
        self.transforms = weights.transforms()
        self.resnet = models.resnet50(weights=weights)
        num_features = self.resnet.fc.in_features
        self.resnet.fc = nn.Identity()
        self.linear1 = nn.Linear(2080, num_output_features)
        self.sig = nn.Sigmoid()
        self.relu = nn.ReLU()
        self.return_head = return_head
        ...
        if self.multitask:
            self.output_head = nn.Linear(num_output_features + self.num_tools, num_classes)
        else:
            self.output_head = nn.Linear(num_output_features, num_classes)

    def forward(self, inputs):
        x, tool_tensor = inputs
        x = self.resnet(x)  # [batch, 2048]
        x = torch.cat([x, tool_tensor], dim=1)  # [batch, 2048 + 32]
        x = self.linear1(x)  # [batch, num_output_features]
        x = self.relu(x)
        if self.return_head:
            x = self.output_head(x)
        return x
