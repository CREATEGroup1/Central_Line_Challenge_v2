...
from DatasetGenerator import CNNDataset, LSTMDataset
...
def getResnetPredictions(data, resnetModel, device, batch_size=64):
    ...
    for i, row in data.iterrows():
        ...
        # Grab the pre-extracted tool vector if it exists (or dummy zeros if not)
        if "Tool vector" in row:
            tool_vector = numpy.array(json.loads(row["Tool vector"])).astype(numpy.float32)
        else:
            tool_vector = numpy.zeros(32, dtype=numpy.float32)

        image_tensors.append(img_tensor)
        tool_tensors.append(torch.tensor(tool_vector))

        # Process in batches
        if len(image_tensors) == batch_size or i == data.index[-1]:
            img_batch = torch.stack(image_tensors).to(device)
            tool_batch = torch.stack(tool_tensors).to(device)
            with torch.no_grad():
                preds = resnetModel((img_batch, tool_batch)).cpu().numpy()
    ...
    return numpy.array(all_predictions)
...
def trainResnet(foldDir, model, training_data, val_data, args, labelName="Overall Task"):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.cnn_batch, shuffle=True, num_workers=1, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.cnn_batch, num_workers=1, pin_memory=True)
    ...
    for epoch in range(args.cnn_epochs):
        ...
        if num_epoch_without_improvement > 0 and num_epoch_without_improvement % 2 == 0:
            lr = adjust_learning_rate(lr, optimizer)

        model.train()
        loss_per_batch, correct_instances, total_instances = [], 0, 0
        start_time = last_logging = time.time()

        for step, ((img, tool), y) in enumerate(train_loader, start=epoch * len(train_loader)):
            img = img.to(gpu, non_blocking=True)
            tool = tool.to(gpu, non_blocking=True)
            ...
            correct_instances += (preds == y).sum().item()
        ...
        model.eval()
        val_loss_per_batch, correct_instances, total_instances = [], 0, 0
        with torch.no_grad():
            for val_step, ((img, tool), y) in enumerate(val_loader):
                img = img.to(gpu, non_blocking=True)
                tool = tool.to(gpu, non_blocking=True)
                y = y.to(gpu, non_blocking=True)
                x = (img, tool)
        ...
        print(f"Epoch {epoch}: Train acc: {train_accuracy:.4f}, Val acc: {val_accuracy:.4f}")
        if val_loss < min_loss:
            torch.save({"model": model.state_dict()}, os.path.join(foldDir, "resnet.pth"))
            min_loss = val_loss
            num_epoch_without_improvement = 0
        else:
    ...
    # Final eval
    model.load_state_dict(torch.load(os.path.join(foldDir, "resnet.pth"))["model"])
    model.eval()
    val_loss_per_batch, correct_instances, total_instances = [], 0, 0
    pred_labels, true_labels = [], []

    with torch.no_grad():
        for (img, tool), y in val_loader:
            img = img.to(gpu, non_blocking=True)
            tool = tool.to(gpu, non_blocking=True)
            y = y.to(gpu, non_blocking=True)
            x = (img, tool)
...
def getResnetFeatures(foldDir,set_name,data,resnetModel,device):
    if not os.path.exists(os.path.join(foldDir,"{}_resnet.npy".format(set_name))):
        res_preds = getResnetPredictions(data, resnetModel, device, batch_size=64)
...
def trainLSTM(foldDir,lstm_model,train_data,val_data,train_res_preds,val_res_preds,args,labelName="Overall Task"):
    ...
    resultsDict = {"Model name": "LSTM",
                   ...
                   "Final Val loss": [],
                   "Final Val accuracy": []}
    train_dataset = LSTMDataset(train_data, labelName, classes, train_res_preds, sequence_length=args.lstm_sequence_length, balance=args.balance_lstm)
    val_dataset = LSTMDataset(val_data, labelName, classes, val_res_preds, sequence_length=args.lstm_sequence_length, balance=False)
...
def main(args):
    if args.save_location == "":
        ...
        bbox_dim = 32
        resnetModel = network.createCNNModel(num_input_features, num_classes).to(gpu)
        if not os.path.exists(os.path.join(foldDir, "resnet.pth")):
            print("RETRAINING RESNET!!!")
